...
import logging
from .attacks import SelfReplicationAttack, GoalDriftAttack, AgentCollusionAttack
from .detectors import ReplicationDetector, GoalDriftAnalyzer, CollusionPatternDetector
...
class RogueAgentOrchestrator:
    ...
    def _detect_self_replication(self, session):
        return ReplicationDetector().detect_unauthorized_replication(
            session.original_agent, session.spawned_agents, session.replication_count
        )
    ...
    def _detect_agent_collusion(self, session):
        return CollusionPatternDetector().detect_collusion_patterns(
            session.collusion_patterns, session.coalition_detected
        )
