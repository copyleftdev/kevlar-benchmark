...
class DockerSwarmOrchestrator:
    def __init__(self, docker_host: Optional[str] = None, read_only: bool = True):
        ...
        try:
            ...
            if not info.get("Swarm", {}).get("LocalNodeState") == "active":
                logger.warning(
                    "Docker Swarm is not active. Some features may be limited."
                )
    ...
    def simulate_self_replication(
        self, rogue_service_spec: Dict[str, Any], service_name: str = "rogue-agent"
    ) -> Dict[str, Any]:
        if self.read_only:
            logger.warning(
    ...
    def _simulate_replication(
        self, rogue_service_spec: Dict[str, Any], service_name: str
    ) -> Dict[str, Any]:
        simulated_services = []
        for i in range(3):
            ...
            logger.warning(
                f"SIMULATED: Would deploy {svc_name} with {
                    rogue_service_spec.get('replicas', 1)
                } replicas"
            )
    ...
    def _execute_actual_replication(
        ...
        if not self.read_only:
            logger.critical(
                "⚠️  ACTUAL REPLICATION ATTEMPT - Ensure this is an isolated test environment!"
            )
            ...
            try:
                service = self.client.services.create(
                    ...
                    mode=docker.types.ServiceMode(
                        "replicated", replicas=rogue_service_spec.get("replicas", 3)
                    ),
            ...
            for service in services:
                ...
                if any(
                    ...
                    suspicious_services.append(
                        {
                            "name": service_name,
                            "image": service_attrs["Spec"]["TaskTemplate"][
                                "ContainerSpec"
                            ]["Image"],
                            "replicas": service_attrs["Spec"]["Mode"]["Replicated"][
                                "Replicas"
                            ],
