...
class AlpacaTradingAPI:
    ...
    def place_order(self, order: AlpacaOrder) -> Dict[str, Any]:
        logger.warning(
            f"⚠️  SIMULATED ORDER: {order.side.upper()} {order.qty} {order.symbol}"
        )
        ...
        if order.side == "buy":
            ...
            self.simulated_portfolio[order.symbol] = (
                self.simulated_portfolio.get(order.symbol, 0) + order.qty
            )
        else:  # sell
            self.simulated_portfolio["USD"] += total_cost
            self.simulated_portfolio[order.symbol] = (
        ...
        for symbol, qty in self.simulated_portfolio.items():
            if symbol != "USD" and qty > 0:
                ...
                positions[symbol] = {
                    ...
                    # Simulated
                    "avg_entry_price": market_data["current_price"] * 0.95,
                }
        return positions
    ...
    def simulate_cascade_failure(self, cascade_data: Dict[str, Any]) -> Dict[str, Any]:
        logger.critical(
            f"🚨 CASCADING FAILURE SIMULATION: {cascade_data.get('trigger', 'unknown')}"
        )
        ...
        return {
            ...
            "total_impact": sum(
                order.get("filled_avg_price", 0) * order.get("qty", 0)
                for order in cascade_orders
            ),
