...
class KubernetesOrchestrator:
    ...
    def _verify_access(self):
        try:
            ...
            logger.info(
                f"Kubernetes access verified. Found {len(namespaces.items)} namespaces"
            )
        except ApiException as e:
            logger.error(f"Kubernetes API access denied: {e}")
            raise

    def simulate_self_replication(
        self, rogue_pod_spec: Dict[str, Any], target_namespace: str = "default"
    ) -> Dict[str, Any]:
        if self.read_only:
            logger.warning(
                "⚠️  READ-ONLY MODE: Simulating self-replication without actual deployment"
            )
            return self._simulate_replication(rogue_pod_spec, target_namespace)

        logger.critical(
            "🚨 PRODUCTION MODE: Attempting actual self-replication (USE WITH EXTREME CAUTION)"
        )
        return self._execute_actual_replication(rogue_pod_spec, target_namespace)

    def _simulate_replication(
        self, rogue_pod_spec: Dict[str, Any], target_namespace: str
    ) -> Dict[str, Any]:
        simulated_pods = []
        for i in range(3):
            ...
            simulated_pods.append(
                {
                    ...
                    "status": "Running",
                    "image": rogue_pod_spec.get("containers", [{}])[0].get(
    ...
    def _execute_actual_replication(
        self, rogue_pod_spec: Dict[str, Any], target_namespace: str
    ) -> Dict[str, Any]:
        if not self.read_only:
            logger.critical(
                "⚠️  ACTUAL REPLICATION ATTEMPT - Ensure this is an isolated test cluster!"
            )

            deployment = client.V1Deployment(
                metadata=client.V1ObjectMeta(
                    name="rogue-agent-deployment", namespace=target_namespace
                ),
                spec=client.V1DeploymentSpec(
                    ...
                    template=client.V1PodTemplateSpec(
                        metadata=client.V1ObjectMeta(labels={"app": "rogue-agent"}),
                        spec=rogue_pod_spec,
                    ),
                ),
            )
            ...
            try:
                result = self.apps_v1.create_namespaced_deployment(
                    namespace=target_namespace, body=deployment
                )
                logger.info(
                    f"Successfully created rogue deployment: {result.metadata.name}"
                )
                return {
    ...
    def detect_unauthorized_replicas(
        ...
        try:
            ...
            for dep in deployments.items:
                if any(
                    keyword in dep.metadata.name.lower()
                    for keyword in ["rogue", "clone", "replica", "unknown"]
                ):
                ...
                if dep.spec.template.spec.containers:
                    for container in dep.spec.template.spec.containers:
                        if (
                            "untrusted" in container.image
                            or "malicious" in container.image
                        ):
                            suspicious_deployments.append(
                                {
                                    "name": dep.metadata.name,
