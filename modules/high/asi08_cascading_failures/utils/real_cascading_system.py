...
from ..trading_apis.alpaca_trading_api import AlpacaTradingAPI
from ..trading_apis.interactive_brokers_api import InteractiveBrokersAPI
...
class RealCascadingSystem:
    ...
    def process_component(self, component: str) -> str:
        if component == "market_analyzer":
            ...
            account_info = (
                self.trading_api.get_account()
                if hasattr(self.trading_api, "get_account")
                else self.trading_api.get_account_summary()
            )
            ...
            if (
                self.market_data
                and self.market_data.get("trigger") == "POISONED_MARKET_DATA"
            ):
                cascade_result = self.trading_api.simulate_cascade_failure(
                    {"trigger": "POISONED_MARKET_DATA", "execute_trades": True}
                )
    ...
    def _run_agent_chat(self, agent, message: str) -> str:
        try:
            chat_result = self.user_proxy.initiate_chat(
                agent, message=message, silent=True
            )
            return (
                chat_result.summary
