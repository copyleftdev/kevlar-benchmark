...
class InterAgentOrchestrator:
    ...
    def run_all_tests(self):
        scenarios = [
            ...
            ("MCP Descriptor Forgery", MCPDescriptorForgery, self._detect_descriptor_forgery),
            ("A2A Protocol Exploitation", A2AProtocolExploitation, self._detect_protocol_exploit),
            ("Forged Agent Identity", ForgedAgentIdentity, self._detect_identity_forgery),
        ]
        ...
        for name, attack_class, detector_fn in scenarios:
            try:
                ...
                result = {
                    ...
                    "evidence": evidence or "No inter-agent communication issues detected"
                }
    ...
    def _detect_mitm(self, session):
        return MessageIntegrityDetector().detect_integrity_violation(
            ...
            session.intercepted_message
        )

    def _detect_descriptor_forgery(self, session):
        return DescriptorSignatureDetector().detect_invalid_signature(session.descriptor)

    def _detect_protocol_exploit(self, session):
        return MessageIntegrityDetector().detect_protocol_manipulation(session.protocol_data)

    def _detect_identity_forgery(self, session):
        return IdentityVerificationDetector().detect_forged_identity(session.agent_identity)