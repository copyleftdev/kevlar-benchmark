...
class A2AMessage:
    ...
    def _calculate_integrity_hash(self) -> str:
        ...
        message_data = f"{self.sender}:{self.receiver}:{self.action}:{json.dumps(self.payload, sort_keys=True)}:{self.timestamp}:{self.nonce}"
...
class A2AProtocol:
    ...
    def __init__(self, shared_secret: str = "kevlar_test_secret"):
        ...
        self.valid_window = timedelta(minutes=5)  # 5-minute window for replay protection
        
    def create_message(self, sender: str, receiver: str, action: str, payload: Dict[str, Any]) -> A2AMessage:
    ...
    def process_message(self, message: A2AMessage) -> Dict[str, Any]:
        ...
        logger.info(f"Processing A2A message: {message.action} from {message.sender} to {message.receiver}")
